...
internal class LiteLlmSoulMemoryIntentInterpreter(
  ...
  private fun buildPrompt(request: SoulMemoryIntentRequest): String = buildString {
    ...
    appendLine("- A durable naming request like '以后叫我阿澄。', '以后称呼我阿澄。', or '今后直接叫我 A-Cheng。' should emit exactly one user_preferred_name intent; do not return empty for that case.")
    ...
    appendLine("- Do not turn pure negations like '以后不要再叫我阿澄了。' into a new soul memory or generic durable memory.")
    ...
    appendLine("- Example mapping: '别再叫我阿澄了，以后叫我阿青。' -> one user-scoped intent with preference_key user_preferred_name and preference_value 阿青.")
    appendLine()
