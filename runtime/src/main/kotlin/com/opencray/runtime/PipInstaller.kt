...
class PipInstaller(
  ...
  fun install(request: PipInstallRequest): ExecutionResult {
    ...
    val proc = try {
      ProcessBuilder(cmd)
        .directory(request.workspaceRoot.toFile())
    ...
    // Runner output is a single JSON blob on stdout; keep capture simple.
