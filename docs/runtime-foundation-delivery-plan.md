...
Current provider-reconnect verification update: the production file-backed managed-process registry, backend router, and E2B provider-native controller are now covered together across a simulated process/controller identity change. The integration test verifies envd `Connect` selection, rebuilt-owner restore metadata, attached live state, and durable terminal writeback. Its provider transport is deterministic test transport, so a live E2B endpoint exercised across Android runtime-process death remains pending.
