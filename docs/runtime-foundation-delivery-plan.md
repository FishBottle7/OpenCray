...
Current device-verification update: `RuntimeServiceProcessIsolationTest` now binds both target services, verifies remote v2 controllers and independent process ids, exercises a serialized write, then kills `:runtime_controller`, waits for the stale owner lease to expire, invokes the production `owner_lease_expired` repair gateway, and checks that the rebuilt detached owner has a new OS/process-start identity while retaining the target's durable controller identity. The harness is present but still needs execution on a connected emulator or device before this plan can claim process-death verification complete. AndroidTest source-set compilation is currently blocked by unrelated existing UI tests that reference removed resource ids; the compiler reports no error from the new runtime test itself.
