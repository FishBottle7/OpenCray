...
The shared `DirectoryDurableTextStorage` path now also wraps file read/write/delete/update operations in per-file JVM locks plus sidecar OS file locks before atomic replace, and key read-modify-write stores for queue snapshots, run records, checkpoints, schedules, memory, notification-delivery dedupe, session supplements, and subagent handles now use that single-file update primitive instead of splitting load/mutate/save across separate process locks. Scheduled run-record pruning/deletion is covered by the same single-file update path, so deleting one schedule's run history no longer writes a stale pre-delete snapshot over another schedule's newer run record. Runtime notification-delivery dedupe writes are covered too, so foreground/service/repair overlaps no longer lose another run's terminal-notification fingerprint by saving an older dedupe snapshot. Mid-loop supplement append/consume is covered as well, so a service-side consume cannot erase a newer supplement injected by another foreground or repair path. The direct-file run journal append path is now process-safe as well: append/list/clear operations serialize through a session-level sidecar OS lock, allocate sequence numbers from disk under that lock, and write entries through temp-file atomic moves.
...
- the default caller-side runtime-service client now has no low-level loopback HTTP read or command fallback in `main` code; projection snapshot fallback remains the only built-in client fallback below the service-backed gateway layer
- service-backed chat/skills/settings writes now require a binder-backed dispatch path, and the old low-level caller-side command fallback escape hatch has been removed from `main` code entirely
...
- the recurring repair precheck is now target-aware: persisted scheduled-task metadata, detached-control metadata, checkpoint-linked queue tasks, durable background subagent handles, managed-process reconnect evidence tied to scheduled/detached queue tasks, queue-stored `resume_reconnect_process` evidence with process id and retry-after metadata, and journal-tail-only evidence that can be matched back to a scheduled/detached queue task can wake `DETACHED_BACKGROUND`, while checkpoint-only, run-record-only, and unmatched non-terminal journal-tail-only repair remains conservative `INTERACTIVE`; future reconnect retry evidence schedules a delayed repair at `retryAfterEpochMs` instead of waiting for the next hourly periodic pass, same-run/task repair evidence can no longer wake the service ahead of that reconnect retry deadline, and service-side repair projection now records and re-enqueues the next pending repair deadline explicitly
...
- broader autonomous repair policy for interrupted runs beyond the current target-aware typed-evidence `WorkManager` precheck, managed-process reconnect delayed repair, owner-lease-expiry delayed repair, periodic repair registration, and `ACTION_RESUME_INTERRUPTED_RUNS` wake-and-resume scan
...
- notification actions for approval, open, retry/manual scheduled-run, disable-schedule, and snooze-schedule flows
