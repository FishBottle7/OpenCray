...
- safety policy settings now also persist as one runtime-root durable snapshot instead of defaulting to multi-key `SharedPreferences`; existing SharedPreferences state is migrated when the durable snapshot is empty, so app-process settings saves and runtime-process tool-policy reads converge on one process-safe source of truth for automation mode, external access, tool overrides, and subagent context policy
