...
Current device-verification update: `RuntimeServiceProcessIsolationTest` now binds both target services, verifies remote v2 controllers and independent process ids, exercises a serialized write, then kills `:runtime_controller`, waits for the stale owner lease to expire, invokes the production `owner_lease_expired` repair gateway, and checks that the rebuilt detached owner has a new OS/process-start identity while retaining the target's durable controller identity. The harness now lives under `runtimeIsolationAndroidTest` and compiles independently with `-PruntimeIsolationAndroidTestOnly=true`, so stale UI instrumentation resource ids no longer block this runtime verification path. It still needs execution on a connected emulator or device before this plan can claim process-death verification complete.
Current provider-reconnect verification update: the production file-backed managed-process registry, backend router, and E2B provider-native controller are now covered together across a simulated process/controller identity change. The integration test verifies envd `Connect` selection, rebuilt-owner restore metadata, attached live state, and durable terminal writeback. Its provider transport is deterministic test transport, so live E2B plus Android process-death evidence remains pending.
