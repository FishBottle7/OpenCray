...
Chat workspace transcript append paths for direct message append and submitted-turn creation now also reload the current workspace record under the file-backed update lock and preserve monotonic workspace/session timestamps, so service/runtime chat appends do not drop newer foreground or repair-owned extensions while adding transcript messages.
