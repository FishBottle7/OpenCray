...
for the requirement-to-code-to-test matrix and the explicit process-death contract. The detailed
history below records incremental implementation and may describe earlier slices as partial.

Closure update: the scheduled-task surface now includes policy-pipelined
`ScheduledTaskCreate/List/Get/Update/RunNow/Snooze/Delete` tools, Binder/loopback/platform
management gateways, and a Flutter list/detail surface with real enable, run-now, snooze, history,
and schedule-id deep-link behavior. Notification settings now control seven application event
types through the same process-safe store consumed by detached notification delivery; the Android
foreground-service notice is correctly treated as required system UI, not an application toggle.
Approval and schedule actions are also exercised by sending their real immutable service
`PendingIntent`s on API 35.

### Incremental Implementation History

Phase 1 complete; Phase 2 approval-boundary and generalized checkpoint restore slice substantially implemented; in-process detached-owner foundation, a production `:runtime` service shell, foreground keepalive, scheduled wake bridges, a first interrupted-run repair wake path, service-owned approval notification handling, and a dedicated service wake-command dispatcher seam landed. Service recreate now also reuses a runtime-process execution controller plus service-neutral bootstrap assembly instead of re-bootstraping the execution core on every shell instance, the keepalive/foreground shell-control state machines now also survive same-process service recreate behind that retained controller, and the service shell now feeds a lease-based app-visibility heartbeat plus strong-background tier into idle-grace/foreground behavior to reduce software-caused interruption during ordinary backgrounding without reacting to every transient UI stop/start edge immediately. That app-visibility heartbeat now also persists through the runtime storage root's file-backed durable store instead of `SharedPreferences`, so the main process and `:runtime` process read the same process-safe lease record while app-private broadcast still handles fast local fanout. The retained transport coordinator now also disposes the previously bound service-owned gateway bundle on shell rebind, so shell/chat observer registrations do not accumulate across same-process service recreate. Explicit retained-owner replacement now reuses a retained in-process execution core and only rotates runtime-owner lifecycle/access wrappers, so same-process reset no longer drops live session managers, executors, or managed-process continuity merely because the shell asked for a rebuild; full controller disposal still tears that retained core down deterministically even if bootstrap-assembly disposal throws. Runtime routing is now explicit end to end as well: caller-side UI bundles default to `INTERACTIVE`, scheduled wakes and detached-control or scheduled approvals route to `DETACHED_BACKGROUND`, the Android service shell keeps concurrent target-scoped bootstraps instead of one mutable default shell, service-backed chat writes now resolve their target from durable queue or checkpoint state before dispatch, durable projection fallback is now target-scoped so interactive and detached clients no longer overwrite each other's last snapshot bucket, and service-owned loopback transport now also splits by `RuntimeServiceTarget` so concurrent interactive and detached shells no longer fight over one loopback port or one default server-state bucket. Binder-unavailable Android clients also now reopen the matching target-scoped durable projection store even when no projection store was injected explicitly. The retained-runtime composition path is now narrower too: client/service gateway factories, local-host gateway creation, runtime dependency loading, Flutter/runtime target bridge entrypoints, and the process-scoped execution-controller path all resolve through explicit environment-owned seams instead of deep `Context -> singleton/helper` fallbacks, with execution-controller bootstrap now consuming only a narrowed `RuntimeExecutionDependencies` bag rather than the full runtime context dependency bag. Managed-process restore is now controller-aware inside the same runtime process, and its file-backed registry now serializes session-level read/modify/write operations behind a directory-scoped JVM lock plus OS file lock so concurrent owners do not lose process snapshots while detached and projection-only readers overlap. Runtime-level managed-process routing now also preserves delegate reconnect support for non-Python-runtime processes, so file-backed registry restore does not hide reconnectable backend capability behind the runtime adapter layer. Runtime-service lifecycle diagnostics now record the expected dedicated service process, the observed current process, and any mismatch reason, projection persistence carries that descriptor so binder fallback reads can detect a service/process ownership regression instead of only trusting the manifest, and bootstrap now rejects a mismatched service process before creating the runtime owner. Recovery diagnostics now also stamp initial `runAttempt`, advance it on recovery-aware queue rewrites, and surface checkpoint-driven `recoveredFromCheckpointId` in run snapshots. App-start and boot/package-replaced paths now also keep a unique periodic `WorkManager` repair registered, so interrupted-run and schedule reconciliation are no longer limited to the first one-shot wake after process start or broadcast delivery; that repair precheck now resolves persisted queue/checkpoint/subagent, run-record-only, and non-terminal journal-tail evidence into `INTERACTIVE` and `DETACHED_BACKGROUND` targets before waking `:runtime`, with terminal run records and final journal tails excluded from interrupted-work detection. Schedule failure/skip notifications now also expose service-backed retry/manual-run, disable, and snooze actions that wake `:runtime`, mutate durable schedule state where needed, and re-sync registered triggers from the service-owned dispatcher; accepted scheduled-run notifications now also expose a per-run Cancel action that reuses the service-owned chat-write interrupt wake command instead of adding a separate cancellation path. Interrupted terminal notifications now also expose a Retry action that builds a target-scoped service PendingIntent for the existing `RetryChatRun` wake command and carries the terminal notification task id into the wake dispatcher, so explicit user retry can wake `:runtime`, start retry dispatch, and clear the stale interrupted notification without first reopening the UI. That schedule re-sync path is now also serialized by a runtime-root cross-process lock around spec enumeration, prior synced-id readback, cancel/sync fanout, and synced-id persistence, so app-process bootstrap, repair, and runtime-side notification actions do not race each other into stale trigger registration state. Schedule notification taps now open the Notifications & Background settings detail entrypoint and preserve `notificationScheduleId` for later schedule-specific surfaces. Detached Android component/process ownership and cross-process controller reads/writes have landed, and the API 35 process-isolation harness now verifies remote `BinderProxy`, independent target PIDs, detached-process kill/recreate, owner-lease-expiry repair, and durable controller identity continuity. Physical-device repetition and credentialed cloud-provider managed-process reconnect evidence remain pending before the stronger controller-owned tier is considered complete.
Current process-ownership update: `INTERACTIVE` now resolves to `OpenCrayAgentRuntimeService` in `:runtime`, while `DETACHED_BACKGROUND` resolves to `OpenCrayDetachedRuntimeService` in the independent `:runtime_controller` process. Each component accepts only its own target, uses its component target for Android null-intent sticky restart, rejects explicit cross-target or invalid-target delivery, validates the target-specific process suffix before runtime-owner bootstrap, and uses a target-specific foreground notification id so one service cannot remove the other service's keepalive notification. Scheduled wakes, repair wakes, notification actions, and target-scoped client binds all inherit this mapping through `RuntimeServiceIntentFactory`, so detached ownership no longer shares the interactive service process. Target-scoped durable projection reads and loopback command fallback remain the process-neutral compatibility path alongside the controller IPC update below.
Current controller-IPC update: `IRuntimeServiceController` v2 preserves the v1 protocol/target/projection transact ordering, adds a capability bitset plus one bounded serialized write-command entrypoint, and accepts v1 peers as projection-only controllers. Chat, skills, and settings commands share a schema-versioned domain/method/route/query/payload codec with the target-scoped loopback fallback, and typed completed/message/payload results return through the same bounded result codec. The remote adapter negotiates target and capabilities before dispatch, never exposes process-local gateway objects, and routes decoded writes back through the existing `RuntimeServiceBinderEndpoint`, preserving owner-lease checks, chat target forwarding, work-state refresh, and projection persistence. Unsupported capabilities continue to fall through to loopback or wake transport; AIDL v1 remains read-only. API 35 now also binds a pure Android-SDK v1 controller from a separate test-APK process, decodes its projection over a remote `BinderProxy`, and verifies that writes use command fallback without issuing absent v2 capability/write transactions.
Current device-verification update: `RuntimeServiceProcessIsolationTest` has eight API 35 cases. It verifies independent target PIDs and v2 writes, pure-v1 projection-only compatibility plus command fallback, detached kill/recreate through owner-lease-expiry repair, same-run checkpoint continuation, no-checkpoint no-replay interruption, process-external E2B reconnect, a real schedule-notification snooze action reaching the main-process WorkManager owner, and immutable approval Approve/Reject service actions reaching `:runtime_controller` and dismissing their notifications. The isolated suite passes with `8` tests, `0` failures, `0` errors, and `0` skipped. Physical-device repetition remains release evidence.
Current WorkManager-ownership update: WorkManager remains initialized and owned only by the main app process. `:runtime`, `:runtime_controller`, and other secondary processes route scheduled wake/cancel/repair commands and LiteRT model-download enqueue/cancel commands through structured explicit broadcasts to non-exported main-process receivers. Process routing, command codecs, and dispatch are JVM-tested; API 35 also verifies a real `:runtime_controller` schedule mutation reaches the main owner after cancel/reschedule ordering and remains `ENQUEUED` as the expected unique WorkManager wake.
Current process-safe media-registry update: generated media artifact registration in the runtime process and artifact resolution/GC in the app process now share `DirectoryDurableTextStorage.updateText(...)` instead of a process-local JVM lock plus direct truncate write. Registration and sweep merge against the locked current record, write by atomic replacement, and rebuild malformed JSON on the next registration; stale-read and concurrent multi-instance tests verify artifact records are not lost.
Current provider-reconnect verification update: the production file-backed managed-process registry, backend router, and E2B provider-native controller are covered together across both a simulated process/controller identity change and actual Android `:runtime_controller` death. The JVM integration test verifies envd `Connect` selection, rebuilt-owner restore metadata, attached live state, and durable terminal writeback; the API 35 case adds a process-external socket endpoint, production `UrlConnection` framing, two independent runtime PIDs, owner-lease repair, and same-remote-pid reattach. This proves the Android process-death transport/registry chain without cloud credentials; a credentialed E2B cloud endpoint run and physical-device repetition remain release evidence.
Current checkpoint-recovery update: runtime checkpoint emission and recovery now span pre-model request, parsed action batch, committed commentary, committed tool result, supplement ingestion, and finalization completion in addition to approval boundaries. `SessionQueueRestoreTransformer` gives core queue construction a generic pre-normalization recovery hook, and the app recovery transformer joins durable checkpoint/journal/result/approval/managed-process evidence there before core applies conservative interruption to anything still unproven. API 35 now also verifies both one parsed-action finalization continuation across actual detached-process death under the same task/run identity and the complementary no-checkpoint path that remains failed without execution-ordinal advancement or a synthesized recovery checkpoint. Terminal result persistence now clears non-manual prompt checkpoints inside the retained runtime handle after the durable run record write, so host-listener rebind cannot strand a late `finalization_complete` checkpoint; approval and LLM retry-paused results still retain their manual-resume checkpoints. Automatic checkpoint continuation now persists `_host.checkpointResumeAttemptCount` immediately before each actual `checkpoint_resume` execution and stops after three interrupted resume attempts with `RESTART_REQUIRES_EXPLICIT_RETRY`; explicit Retry clears that budget, while durable terminal-result repair still takes priority over the limit. The same count reaches run diagnostics, terminal result metadata, and recovery-journal dedupe evidence. Uncertain in-flight mutation and no-checkpoint cases remain explicit interruption rather than replay.
Current checkpoint-repair preflight update: a budget-exhausted failed task keeps its prompt checkpoint for explicit user Retry, but automatic repair scans suppress checkpoint, non-terminal run-record, journal-tail, and matching typed projection evidence for that task/run identity. This prevents periodic or startup repair from replacing an execution restart loop with a service wake loop. Managed-process and unrelated session evidence remain eligible, and moving the task back to queued through explicit Retry removes the suppression.
Current terminal-projection convergence update: WorkManager preflight and runtime-service bootstrap/resume now derive terminal task/run identities plus final journal-tail identities before merging the last service-side repair projection. Typed projected queue/checkpoint/run/journal/reconnect evidence for an already terminal identity is discarded, preventing an older projection from waking a target after durable state has converged. Local checkpoint and non-terminal journal evidence still remain eligible even when a terminal queue row is retained only for target routing, and a live managed-process registry remains independent durable reconnect evidence.
Current runtime file-mutation update: production tool dispatchers in `:runtime` and `:runtime_controller` now share a runtime-root `FileMutationLockCoordinator`. Each `FileOpsService` holds the same sidecar OS lock across path resolution, rollback checkpoint capture, the full create/write/delete/move batch, commit, and rollback; replacement writes use same-directory temporary files plus atomic move while retaining POSIX permissions when available. `Edit` and `MultiEdit` also keep their source read and resulting write inside that reentrant transaction, closing the previous cross-process stale-read/lost-update window between interactive and detached owners.
Foreground-service wake starts now enter a target-scoped minimal bootstrap foreground state before resolving the runtime environment, assembling the service bootstrap, acquiring the owner lease, or starting transport/execution attachment. The attached runtime shell then adopts the same target notification through its retained foreground controller, while bound-only attachment remains non-foreground. Starts that lose target validation or runtime-owner lease arbitration stop their current Android start id before returning `START_NOT_STICKY`, preserving scheduled lease repair without leaving an Android foreground deadline behind. The isolated API 35 suite passes all seven cases with an empty XML `system-err`, and the cleared device log contains no `ForegroundServiceDidNotStartInTimeException`, remote-service crash, or fatal runtime exception.
...
Target-scoped runtime-owner lease heartbeat now also persists the current held/released owner evidence into the runtime storage root and projection diagnostics, including process/controller/owner/service ids plus heartbeat and expiry timestamps. The lease store now rejects a different owner while the current held lease is unexpired, retained runtime-owner replacement explicitly releases the previous owner lease before writing the new heartbeat, and a runtime-service projection coordinator that cannot acquire the target lease now skips projection snapshot writes instead of overwriting the active owner's target projection. Rejected acquire attempts are now recorded on the held lease with attempted owner/controller/service ids plus holder expiry, and the active owner's next heartbeat carries that conflict into projection diagnostics for binder-unavailable fallback. The same owner check now gates shell attach, service `onStartCommand(...)`, wake-command dispatch, Binder chat/skills/settings writes, service-owned loopback write routes including the GET-backed skill update check, and sticky start-result decisions, so a losing shell does not start transport/observers, mutate keepalive state, expose a cached binder or loopback write endpoint, mutate runtime-owned data, or ask Android to keep restarting it after another owner holds the target lease. This is target-process ownership arbitration; API 35 now verifies controlled detached kill/recreate, lease-expiry repair, and one safe checkpoint-backed active-task continuation cycle, while uncheckpointed active work, physical-device repetition, and broader OEM loss modes remain separate evidence before claiming execution survives every target-process loss.
Runtime-service projection snapshot saves now also use the locked durable update path and retain newer owner-lease plus interrupted-run repair collateral from the current durable record, so an older projection flush does not erase the latest lease conflict, heartbeat, or reconnect repair evidence visible to binder-unavailable diagnostics.
Owner-lease attach denial now also schedules a repair retry at the held lease expiry, and an attached shell that loses the lease before processing a start command now takes the same owner-lease-expiry retry path while still dropping the command. Production service attach/start-result failure reads the durable target lease, enqueues an `owner_lease_expired` repair through WorkManager for `expiresAtEpochMs`, delayed repair work is partitioned by reason so owner-lease retry, managed-process reconnect retry, and other delayed repair sources do not replace each other, and repair preflight treats an already-expired still-held lease as a due target that must be woken through the interrupted-run rescan path rather than merely recording a successful no-op worker run.
...
Skill install manifest direct save now also uses durable text `updateText`, so future skills paths cannot bypass the process-safe manifest update contract with direct `writeText`. Malformed skill install manifests now load as empty manifests and recover through the next locked save/update instead of blocking runtime skill initialization.
...
Per-agent config snapshots now also use the shared durable text storage under each agent private directory, with save/clear routed through the locked update primitive, so service/runtime agent-scope reads no longer depend on direct truncate writes for `agent-config.json`.
Workspace and agent-private `SOUL.md` profile save/clear now also use shared durable text storage, and profile saves merge the current durable frontmatter inside the locked update callback instead of splitting load/merge/write, so foreground personalization updates and service/runtime agent-scope reads no longer depend on direct truncate writes for soul profile documents.
...
Settings image asset index writes now also merge through the locked durable update path and store reads refresh from the durable snapshot, so foreground settings imports and service/projection image-reference reads do not drop another owner's newly imported image asset through a stale in-memory index.
MCP server registry add/remove/enable/disable and built-in server seeding now also mutate the current registry through a store-level atomic update path, so foreground settings and runtime/service MCP surface refreshes do not overwrite another owner's newer server or trust-state entry from a stale registry instance.
MCP registry direct `save`/`clear` entrypoints now also route through the locked durable text update primitive, so fallback callers and tests cannot reintroduce direct `writeText`/`delete` persistence around the atomic registry mutation path.
Session compaction state now also exposes an atomic store update path, and runtime compaction append uses it instead of saving a caller-held pre-append snapshot, so concurrent foreground/service/runtime compaction work preserves another owner's durable summary entry while still writing through the locked file store. Malformed compaction snapshots now load as empty state and recover on the next locked update instead of aborting runtime compaction during service/runtime rebuild.
...
Scheduled trigger sync-state replace/clear mutations now also route through the locked durable update primitive instead of directly writing or deleting the synced-id record, so app bootstrap, repair, and runtime-side notification action resyncs share the same process-safe mutation path after the cross-process resync transaction finishes computing the desired trigger set. Malformed trigger sync-state snapshots now load as an empty synced-id set and are recovered by the next replace, so a bad record cannot block alarm/WorkManager trigger reconciliation.

Runtime notification settings save/clear now also use the locked durable update primitive instead of direct write/delete calls, so foreground settings writes and detached notification-policy reads stay on the same process-safe mutation path for quiet hours, delivery mode, and event-channel toggles.
...
Runtime-root settings migration now treats malformed durable JSON as no usable durable record instead of only checking whether the file is non-empty. Sandbox, E2B sandbox-session, safety policy, live-context mode, network-search, MCP master, LLM config, media/speech, app-language, app-shell navigation, skills enablement, and telemetry/privacy stores can therefore still recover legacy `SharedPreferences` values through the locked durable update path after a corrupt runtime-root snapshot.
...
Reconnect-delayed collateral evidence now also inherits the reconnect hold's runtime execution ownership tier and durable runtime-controller id when those fields were missing, so run-record or journal-tail diagnostics keep the controller anchor that caused managed-process backoff.
Target-scoped projected managed-process reconnect evidence is now also retained when a session already has weaker durable run-record or journal-tail repair evidence. The weaker evidence inherits the projected reconnect deadline, status, recovery state, attempt count, continuation basis, restore scope/decision, ownership tier, durable controller id, and detached target instead of dropping the projection just because any local durable evidence exists.
...
Managed-process registry snapshot persistence now also writes through the shared durable text `updateText` primitive, so start/wait/terminate/reconnect repair snapshot saves no longer depend on direct `writeText` even while the registry keeps its broader controller synchronization lock.
...
Managed-process active restore now also stamps `managedProcessRestoreDecision` into registry restore metadata and carries it through recovery-aware queue metadata, run lifecycle diagnostics, interrupted-run repair evidence, and the runtime-service projection store. The current values distinguish same-process live-controller reattachment, reconnect attempts, reconnect deferral behind retry backoff, and interrupted fallback when no controller/reconnect path is available; this makes repair policy auditable without pretending the pending controller-process ownership tier has landed.

The file-backed managed-process registry now also keeps a process-start-scoped live-controller index in addition to the strict controller-instance scope. If the runtime service recreates a controller inside the same Android process and no provider reconnect factory is available, the new registry can adopt the still-live controller from the same `processStartId`, stamp `managedProcessRestoreDecision=live_controller_reattached`, and continue `read`/`wait`/`terminate` through that controller instead of marking the process interrupted. Provider reconnect still wins when a reconnectable factory exists, and this remains an in-process continuity path rather than cross-process controller ownership.
Reconnectable managed-process restore now also stamps `managedProcessRestoreDecision=reconnect_attempted` after the backend controller is created, so restore scope/current controller metadata survives even if the provider returns a fresh live snapshot instead of preserving the metadata from the reconnect input snapshot.
...
- the planned safe-boundary set and the generic `SessionQueueRestoreTransformer` integration are complete
- unproven in-flight work intentionally remains explicit interruption rather than becoming a replay path
- additional checkpoint kinds are future product extensions only if new tool semantics provide a provably safe boundary
...
- `ACTION_RESUME_INTERRUPTED_RUNS` now preserves its normalized request reason through the typed wake command, service dispatcher, interrupted-run repair result, target-scoped projection store, and shell/chat diagnostics as `requestedRepairReason`; this keeps owner-lease expiry, managed-process reconnect, periodic, app-start, and generic retry wake provenance distinct from `nextRepairReason`, which still describes only the next delayed retry selected from repair evidence
- projection-only WorkManager preflight now consumes the retained `lastInterruptedRunRepair.repairEvidenceBySession` snapshot before using generic runtime work-summary fallback evidence, so binder-unavailable or host-rebuilt repair scans keep typed `MANAGED_PROCESS_RECONNECT` deadlines/status/attempt/scope/decision and durable controller anchors even when the local durable session stores are temporarily missing; if an older or partially degraded projection only has `nextRepairAfterEpochMs`/`nextRepairReason`, that projection-level deadline is still used as a delayed-repair backstop unless typed evidence already supplies an earlier retry, and an already-due projection-level deadline wakes the matching target even when the typed evidence list is absent
- corrupt runtime-service projection snapshots are now treated as missing on load, so binder-unavailable fallback and repair preflight can degrade to other durable evidence instead of crashing on a malformed projection file; the next service-side projection save still overwrites the bad snapshot through the locked update path
- runtime-service bootstrap and explicit interrupted-run resume now also merge the target-scoped projected repair evidence with durable per-session repair evidence before deciding whether to resume or schedule the next delayed repair; terminal run records suppress stale projected reconnect evidence for completed runs, and reconnect backoff inheritance still applies when only a generic run-record or journal-tail durable hint remains
- retained runtime-owner lease heartbeat is now target-scoped and process-safe: the runtime-process owner periodically renews a durable held lease, projection flushes carry that lease into diagnostics, stale owner writes cannot overwrite a newer lease, retained-controller disposal writes a released lease for teardown evidence and mirrors that release into projection diagnostics only when the durable store still confirms the same owner, denied service attach attempts enqueue an owner-lease-expiry repair instead of waiting indefinitely for another external wake, periodic repair preflight rebuilds the earliest future owner-lease-expiry delayed repair from durable held leases if WorkManager state was lost, and an already-expired still-held lease wakes its matching target through `ACTION_RESUME_INTERRUPTED_RUNS` so owner bootstrap plus durable repair scanning is actually retried
...
- runtime notification settings now also persist as one target-neutral durable snapshot under the shared runtime root instead of multi-key `SharedPreferences`, and save/clear now use the locked durable update primitive, so main-process settings writes and detached runtime notification reads share one process-safe source of truth for quiet hours, delivery mode, and event-channel toggles
...
- E2B sandbox-session resume state now also persists as one runtime-root durable snapshot instead of defaulting to multi-key `SharedPreferences`; existing SharedPreferences state is migrated when the durable snapshot is empty, app-process preview probes plus runtime-process sticky reconnect merge their lifecycle fields through one locked read-modify-write transaction, and stale owners only clear the record when the current sandbox id still matches, so preview/session controls and runtime-process reconnect/resume converge without overwriting a newer active session
- E2B sticky workspace sync state now also persists through the shared durable text storage under each workspace's `.opencray/sandbox-sync` directory, so host rebuild or runtime-process recreate reads the reusable local file manifest from a shared durable snapshot and save/clear writes go through the same locked update primitive as other detached-runtime stores
- E2B native sandbox busy/reclaim checks now read durable managed-process registries through projection-only `FileBackedAgentProcessRegistry` instead of direct JSON parsing, so sticky sandbox liveness shares the registry's process-safe lock and normalization path without actively restoring controllers; malformed managed-process registry snapshots now load as empty records, allowing projection-only scans, service bootstrap, or the next managed-process mutation to continue and rewrite a valid locked snapshot instead of crashing during registry initialization
...
- each target still owns its execution controller and executors inside its own runtime service process; interactive and detached ownership are now separated across `:runtime` and `:runtime_controller`, with API 35 kill/recreate evidence for the detached target, while a still-stronger controller process separate from each service process remains a later optional tier
...
- `resume_reconnect_process` is now wired through recovery-aware queue restore for live observation reconnect backoff, including task metadata, lifecycle diagnostics, and `RECOVERY` journal evidence; when the delayed reconnect repair becomes due, an already-retained live session now re-runs the recovery projection against the active managed-process registry, promotes an `attached_live` reconnect hold to `resume_from_checkpoint` only when the durable `ProcessRead`/`ProcessWait` action still matches the same process id, and actually queues that continuation into the executor. Holds without a matching checkpoint remain suspended. Actual checkpoint-resume executions now increment a durable attempt counter and recovery stops after three repeated interruptions until explicit Retry resets the counter; true controller-level continuation without checkpoint replay is still not implemented across runtime-process death
...
Delivery status: complete for UI-independent and target-process-isolated ownership. Own-process
death follows the checkpoint/reconnect/interruption contract in the acceptance matrix; transparent
preservation of uncheckpointed process memory is not claimed.
...
- keep binder/projection fallback semantics explicit across process recreate and v1 peers
...
Provide scheduled execution and recurring automation without reintroducing UI ownership.

### Current implementation status

Already landed across the app, runtime, and Flutter surfaces:
...
- single-main-process WorkManager ownership: secondary runtime processes send structured scheduled-work and LiteRT model-download commands to non-exported main-process receivers instead of calling `WorkManager.getInstance(...)`; routing/codec/dispatch tests plus an API 35 detached schedule mutation that observes the stable main-process unique wake cover this boundary
- generated media artifact registry updates now use the shared process-safe durable text update primitive, so runtime-side media generation and app-side artifact resolution/GC merge under one OS file lock and atomic replacement instead of truncating a snapshot guarded only by a JVM-local lock
...
- policy-pipelined `ScheduledTaskCreate`, `ScheduledTaskList`, `ScheduledTaskGet`, `ScheduledTaskUpdate`, `ScheduledTaskRunNow`, `ScheduledTaskSnooze`, and `ScheduledTaskDelete` tools, including normalized metadata and full host-side trigger re-sync/cleanup behavior
- Binder v2, target-scoped loopback command, local HTTP, Android platform-channel, and Flutter facade support for scheduled-task list/detail/enable/run/snooze management
- a Flutter task list/detail surface with real enabled-state switches, run-now, 15-minute snooze, recent run history, and disabled-state action guards; prompt/trigger editors are intentionally absent because no save gateway is presented for them
- schedule notification open routing to `/settings/scheduled-tasks?scheduleId=...`, plus retry/manual-run, disable, and 15-minute snooze actions for skipped-busy and dispatch-failed schedule outcomes, per-run Cancel for accepted scheduled runs, and terminal interrupted-run Retry with stale-notification dismissal, all routed through stable runtime-service command envelopes; the shared chat-write wake parser now only carries terminal-notification dismissal metadata for Retry, so accepted-run Cancel does not reuse its task id as a stale interrupted-notification id
- approval Approve/Reject and schedule Snooze actions are verified as immutable service `PendingIntent`s that reach `:runtime_controller`; the schedule mutation is observed in the process-safe spec store and the main-process WorkManager owner

Product extensions outside the current delivery gate:

- additional automation trigger families beyond absolute time, relative delay, and RRULE recurrence
- configurable notification snooze presets beyond the shipped fixed 15-minute action
- any future third-process supervisor that truly moves execution ownership out of the target service process
...
- full scheduled-task lifecycle tools and the Flutter management gateway/list/detail surfaces
...
The checklist below is retained as the original implementation map. Its local delivery-gate items
are implemented; imperative wording describes the work that was planned, not remaining work.

### `app/`

- continue hardening process-safe run journal storage and recovery classification
...
This section records the historical first-slice recommendation. That slice and the later service,
checkpoint, scheduling, notification, and repair slices are now delivered for the local gate.
