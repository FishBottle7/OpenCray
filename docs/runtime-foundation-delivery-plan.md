...
Managed-process restore scope now also projects through the same recovery and repair surfaces. Reconnect recovery plans aggregate runtime-registry `managedProcessRestoreScope` evidence into `_host.managedProcessRestoreScope`, recovery-aware queue rewrites copy it into task metadata, lifecycle diagnostics, and durable `RECOVERY` markers, and interrupted-run repair evidence plus runtime-service projection persistence carry the scope through queue-stored and registry-discovered reconnect holds. This makes `same_controller`, `same_process_new_controller`, and `cross_process` reconnect evidence visible without implying cross-process controller ownership is solved.
