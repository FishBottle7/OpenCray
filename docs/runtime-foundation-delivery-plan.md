...
- schedule-side notification actions now also have a runtime-service command path: skipped-busy and dispatch-failed schedule notifications can emit `RUN_SCHEDULE_NOW` or `DISABLE_SCHEDULE` actions through the same stable command envelope, wake the detached runtime target in the foreground, and either re-dispatch the schedule as a normal manual scheduled trigger or disable the durable schedule with trigger re-sync
...
- schedule notification retry/manual-run actions for skipped-busy and dispatch-failed schedule outcomes, plus a disable action that routes through the same runtime-service wake dispatcher to update durable schedule state and trigger registration

Still pending in this phase:

- richer schedule-side notification actions beyond the current open, retry/manual-run, and disable flows, such as per-run cancellation, snooze, and schedule-detail affordances
- broader autonomous repair policy for interrupted interactive runs beyond the current `WorkManager` precheck, periodic repair registration, and `ACTION_RESUME_INTERRUPTED_RUNS` wake-and-resume scan
