...
E2B sticky workspace sync state now also stores its reusable workspace file manifest through `DirectoryDurableTextStorage` under the workspace `.opencray/sandbox-sync` directory, so load/save/clear share the same sidecar file-lock plus atomic-replace behavior as the other process-safe runtime stores.
...
- E2B sticky workspace sync state now also persists through the shared durable text storage under each workspace's `.opencray/sandbox-sync` directory, so host rebuild or runtime-process recreate reads and writes the reusable local file manifest through the same sidecar file-lock plus atomic-replace primitive as other detached-runtime stores
