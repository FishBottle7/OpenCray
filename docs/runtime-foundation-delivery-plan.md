...
- schedule notification open/detail routing to the Notifications & Background settings entrypoint with `notificationScheduleId` launch metadata, plus retry/manual-run, disable, and 15-minute snooze actions for skipped-busy and dispatch-failed schedule outcomes, per-run Cancel for accepted scheduled runs, and terminal interrupted-run Retry, all routed through stable runtime-service command envelopes
