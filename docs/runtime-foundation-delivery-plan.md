...
Current device-verification update: `RuntimeServiceProcessIsolationTest` binds both target services, verifies remote v2 controllers and independent process ids, exercises a serialized write, then kills `:runtime_controller`, waits for the stale owner lease to expire, invokes the production `owner_lease_expired` repair gateway, and checks that the rebuilt detached owner has a new OS/process-start identity while retaining the target's durable controller identity. The isolated `runtimeIsolationAndroidTest` suite passed both cases on the API 35 `OpenCrayRuntimeIsolationApi35` x86_64 emulator (`2` tests, `0` failures, `0` skipped), so emulator process-death verification is complete and no longer blocked by stale UI instrumentation resource ids. Physical-device repetition remains release evidence.
Current WorkManager-ownership update: WorkManager remains initialized and owned only by the main app process. `:runtime`, `:runtime_controller`, and other secondary processes route scheduled wake/cancel/repair commands and LiteRT model-download enqueue/cancel commands through structured explicit broadcasts to non-exported main-process receivers. Process routing, command codecs, and dispatch are JVM-tested, and the API 35 process-isolation run confirms runtime bootstrap no longer attempts to initialize an independent WorkManager owner.
