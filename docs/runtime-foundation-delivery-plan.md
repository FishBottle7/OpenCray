...
- interrupted restore of checkpoint-backed `ProcessRead` or `ProcessWait` observation now also uses `resume_from_checkpoint` in the narrow safe case above, while `connecting`, `retry_scheduled`, `failed_terminal`, or otherwise ambiguous reconnect states still stay in explicit interrupted recovery and project `hasAutoResumeEligibleManagedProcesses=false`
...
- `resume_reconnect_process` remains a projected recovery intent for live managed-process state, but true reconnect without checkpoint replay is still not implemented end-to-end across process death
...
- shared durable text storage now uses per-file sidecar OS locks around read/write/delete/update, and key JSON read-modify-write stores built on `DirectoryDurableTextStorage` now update under one file lock instead of split load/save operations
...
- the recurring repair precheck is now target-aware: persisted scheduled-task metadata, detached-control metadata, checkpoint-linked queue tasks, durable background subagent handles, managed-process reconnect evidence tied to scheduled/detached queue tasks, and journal-tail-only evidence that can be matched back to a scheduled/detached queue task can wake `DETACHED_BACKGROUND`, while checkpoint-only, run-record-only, and unmatched non-terminal journal-tail-only repair remains conservative `INTERACTIVE`; future reconnect retry evidence schedules a delayed repair at `retryAfterEpochMs` instead of waiting for the next hourly periodic pass
...
- broader autonomous repair policy for interrupted runs beyond the current target-aware typed-evidence `WorkManager` precheck, managed-process reconnect delayed repair, periodic repair registration, and `ACTION_RESUME_INTERRUPTED_RUNS` wake-and-resume scan
