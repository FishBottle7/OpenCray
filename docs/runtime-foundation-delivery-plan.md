...
- the recurring repair precheck is now target-aware: persisted scheduled-task metadata, detached-control metadata, checkpoint-linked queue tasks, durable background subagent handles, and journal-tail-only evidence that can be matched back to a scheduled/detached queue task can wake `DETACHED_BACKGROUND`, while checkpoint-only, run-record-only, and unmatched non-terminal journal-tail-only repair remains conservative `INTERACTIVE`
...
- broader autonomous repair policy for interrupted runs beyond the current target-aware typed-evidence `WorkManager` precheck, periodic repair registration, and `ACTION_RESUME_INTERRUPTED_RUNS` wake-and-resume scan
