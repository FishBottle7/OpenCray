...
- E2B native sandbox busy/reclaim checks now read durable managed-process registries through projection-only `FileBackedAgentProcessRegistry` instead of direct JSON parsing, so sticky sandbox liveness shares the registry's process-safe lock and normalization path without actively restoring controllers
