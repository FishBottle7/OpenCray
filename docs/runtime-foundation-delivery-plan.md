...
- the existing repair worker can now preflight interrupted interactive repair candidates from non-terminal queue snapshots, prompt checkpoints, durable background subagent handles, or durable run-record-only evidence and wake the runtime service with `ACTION_RESUME_INTERRUPTED_RUNS`
- runtime-service interrupted-run repair and bootstrap session scan now reuse that same durable per-session repair predicate, so queue-snapshot, prompt-checkpoint, durable background-subagent, and run-record-only recovery candidates no longer depend on whether the wake path started from WorkManager preflight or from the service-side rescan itself
...
- interrupted restore of checkpoint-backed `ProcessRead` or `ProcessWait` observation now also uses `resume_from_checkpoint` in the narrow safe case above, while `connecting`, `retry_scheduled`, `failed_terminal`, or otherwise ambiguous reconnect states still stay in explicit interrupted recovery
...
- the recurring repair precheck is now target-aware: persisted scheduled-task metadata, detached-control metadata, checkpoint-linked queue tasks, and durable background subagent handles can wake `DETACHED_BACKGROUND`, while checkpoint-only and run-record-only repair remains conservative `INTERACTIVE`
...
- broader autonomous repair policy for interrupted runs beyond the current target-aware `WorkManager` precheck, periodic repair registration, and `ACTION_RESUME_INTERRUPTED_RUNS` wake-and-resume scan
