...
- schedule notification retry/manual-run, disable, and 15-minute snooze actions for skipped-busy and dispatch-failed schedule outcomes, plus per-run Cancel for accepted scheduled runs, all routed through stable runtime-service command envelopes

Still pending in this phase:

- richer schedule-side notification actions beyond the current open, retry/manual-run, disable, fixed snooze, and accepted-run Cancel flows, such as schedule-detail affordances
