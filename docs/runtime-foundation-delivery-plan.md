...
Current controller-IPC update: a versioned read-only `IRuntimeServiceController` AIDL contract now exposes protocol version, owning runtime target, and a bounded serialized runtime-service projection. The binding client accepts a remote `BinderProxy` only after protocol/target negotiation, decodes the shared process-safe projection schema, and reports a real binder connection instead of `invalid_binder`; mismatched or unsupported binders retain the existing fallback state. Process-local shell/chat/skills/settings gateway objects are never returned by the remote adapter, so writes still use the target-scoped loopback or wake fallback until explicit serializable command methods are added to the controller protocol.
