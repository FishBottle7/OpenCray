...
Current device-verification update: `RuntimeServiceProcessIsolationTest` binds both target services, verifies remote v2 controllers and independent process ids, exercises a serialized write, then kills `:runtime_controller`, waits for the stale owner lease to expire, invokes the production `owner_lease_expired` repair gateway, and checks that the rebuilt detached owner has a new OS/process-start identity while retaining the target's durable controller identity. A third case drives a detached schedule snooze through `:runtime_controller` and observes the resulting stable unique wake in the main-process WorkManager. A fourth case persists a `RUNNING` detached task plus an `ACTION_BATCH_PARSED` checkpoint while the first detached process is alive, kills that process, and verifies the rebuilt PID completes the same task/run as `checkpoint_resume` with execution ordinal advancement and `finalization_complete` terminal evidence. A fifth case keeps a Connect-compatible envd endpoint alive in the instrumentation process, lets the first detached PID attach a persisted E2B managed process, kills that PID, and verifies the repaired PID issues a second production `UrlConnection` `Connect` for the same remote pid with incremented reconnect attempts and `cross_process`/`reconnect_attempted`/`attached_live` durable evidence. A sixth case binds that pure-v1 remote controller, verifies projection-only chat/skills/settings capability behavior, and observes the Android client route a write through command fallback. A seventh case persists a detached `RUNNING` task with execution ordinal `1` and no checkpoint, kills `:runtime_controller`, and verifies the repaired owner writes `FAILED`/`RESTART_REQUIRES_EXPLICIT_RETRY` with `runAttempt=2` while execution ordinal stays `1` and no recovery checkpoint appears. The isolated `runtimeIsolationAndroidTest` suite passes all seven cases on the API 35 `OpenCrayRuntimeIsolationApi35` x86_64 emulator (`7` tests, `0` failures, `0` errors, `0` skipped), so remote-v1 compatibility, emulator process-death, safe checkpoint continuation, no-checkpoint no-replay interruption, process-external managed-process reconnect, and cross-process scheduled-work delivery verification are complete and no longer blocked by stale UI instrumentation resource ids. Physical-device repetition remains release evidence.
...
Future extensions outside the current delivery gate:
