...
- schedule notification retry/manual-run, disable, and 15-minute snooze actions for skipped-busy and dispatch-failed schedule outcomes, all routed through the same runtime-service wake dispatcher and stable command envelope

Still pending in this phase:

- richer schedule-side notification actions beyond the current open, retry/manual-run, disable, and fixed snooze flows, such as per-run cancellation and schedule-detail affordances
