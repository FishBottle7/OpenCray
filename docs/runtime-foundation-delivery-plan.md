...
- the binder-backed service chat gateway now serves runtime snapshot plus memory/soul debug reads, search/slice helpers, and memory-debug actions from projection-backed local stores and service-owned observer fanout rather than delegating those surfaces through `OpenCrayHostRuntime`; the primary chat snapshot and per-run snapshot/wait surfaces stay host-backed for now because the projection surface still lacks full message/rendered-event and run-inspector metadata parity
...
- personalization load/save/reset inside the service-owned settings gateway now terminate at `LocalPersonalizationFacade`, and app-language persistence now also writes through a service-owned app-language access seam; the remaining host coupling on that path is narrowed to localized runtime refresh plus chat/skills/settings snapshot fanout, so binder-connected reads no longer need to route the language write itself back through `OpenCrayHostRuntime`
