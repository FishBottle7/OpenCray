...
Target-scoped runtime-owner lease heartbeat now also persists the current held/released owner evidence into the runtime storage root and projection diagnostics, including process/controller/owner/service ids plus heartbeat and expiry timestamps. The lease store now rejects a different owner while the current held lease is unexpired, and retained runtime-owner replacement explicitly releases the previous owner lease before writing the new heartbeat. This is still ownership arbitration for the current `:runtime` process boundary, not a separate controller-process ownership tier.
