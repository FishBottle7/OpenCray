...
- projection-only WorkManager preflight now consumes the retained `lastInterruptedRunRepair.repairEvidenceBySession` snapshot before using generic runtime work-summary fallback evidence, so binder-unavailable or host-rebuilt repair scans keep typed `MANAGED_PROCESS_RECONNECT` deadlines/status/attempt/scope/decision and durable controller anchors even when the local durable session stores are temporarily missing
