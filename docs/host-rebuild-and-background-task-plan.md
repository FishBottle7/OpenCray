...
- E2B sticky workspace sync state now stores the reusable local file manifest through the shared durable text storage under the workspace `.opencray/sandbox-sync` directory, so sticky sandbox reconnect and host rebuild paths no longer read/write that hint through a separate direct-file persistence path
