...
- interrupted terminal notifications can now wake the resolved runtime target through the existing `RetryChatRun` command and clear the stale interrupted notification after dispatch, so explicit user retry after a host rebuild or service recreate no longer depends on first reopening the chat UI
