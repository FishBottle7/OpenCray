...
Current owner-lease repair update: attach-denied service starts and already-attached shells that lose the lease before start-command execution now read the held target lease and enqueue an `owner_lease_expired` delayed repair for `expiresAtEpochMs`, while delayed repair work names are partitioned by reason so owner-lease expiry retries do not replace managed-process reconnect retries.
