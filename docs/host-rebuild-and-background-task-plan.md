...
- controller-level managed-process reconnect is implemented for reconnectable backends, including the runtime routed-factory path when the default delegate supports reconnect; detached ownership now runs in `:runtime_controller`, versioned read/write controller IPC is implemented, and API 35 detached process kill/recreate plus one safe checkpoint-backed active-task continuation now pass while physical-device and live-provider reconnect verification remain open
