...
Current checkpoint-recovery update: prompt execution now emits six general safe-boundary kinds in addition to approval state, including pre-model, parsed-action, commentary, committed-tool-result, ingested-supplement, and completed-finalization checkpoints. Core queue construction invokes the app-supplied recovery transformer before its conservative restart normalization, so checkpoint-aware recovery is an explicit core extension point rather than a post-construction queue mutation; the app planner still owns Android/runtime-specific evidence joins and leaves unproven work interrupted. API 35 now verifies a parsed final action resumes after actual detached-process death with a new PID, execution ordinal advancement, and durable `finalization_complete` result metadata. Actual checkpoint-resume executions now persist a separate attempt count, stop automatic recovery after three repeated interrupted continuations, and require explicit Retry to reset the budget; durable terminal-result restoration remains higher priority than the loop guard.
...
Run journal legacy normalization repair now also rewrites repaired entries through temp-file atomic replace while holding the session journal lock, so host-rebuild replay repair no longer directly truncates an existing journal entry.
Run journal presence checks now also validate decoded entries while holding that lock, so corrupt-only journal directories do not make host bootstrap, repair enumeration, or empty-session discard treat a session as having recoverable runtime history.
Current ownership-evidence update: each target now has a process-safe runtime-owner lease heartbeat that records held/released owner evidence with process, controller, owner, service, heartbeat, and expiry fields. Different owners can no longer overwrite a still-held unexpired lease, rejected acquire attempts are recorded on the held lease with attempted owner/controller/service ids, retained runtime-owner replacement releases the previous owner lease before saving the new owner heartbeat, a coordinator that fails to acquire the target lease skips projection writes instead of replacing the active owner's projection, attach-denied shells now tear down their fresh shell/transport/execution bootstrap immediately instead of leaving a failed detached shell cached in-process, retained projection/notification observers only activate after a shell has actually attached with the lease, and non-owner service shells skip shell attach, start-command handling, wake-command dispatch, binder chat/skills/settings writes, and sticky restart requests. This narrows host-rebuild diagnostics and arbitration for retained-owner continuity, attach-denied ownership conflicts, and controller teardown across both target processes; it does not by itself make in-memory execution survive target-process death.
Current owner-lease repair update: attach-denied service starts and already-attached shells that lose the lease before start-command execution now read the held target lease and enqueue an `owner_lease_expired` delayed repair for `expiresAtEpochMs`, periodic repair preflight also scans durable held leases and rebuilds the earliest future owner-lease-expiry delayed repair if WorkManager state was lost, an already-expired still-held lease is routed as a due target through the interrupted-run rescan wake path, and delayed repair work names are partitioned by reason so owner-lease expiry retries do not replace managed-process reconnect retries.
The shared durable text store now also uses per-file sidecar OS locks for read/write/delete/update, and key read-modify-write JSON stores for queue snapshots, run records, checkpoints, schedules, transcript fallback, memory, notification-delivery dedupe, session supplements, subagent handles, skill install manifests, sandbox settings, E2B sandbox-session resume state, safety policy settings, live-context mode settings, network-search settings, MCP master settings, LLM config settings, media/speech settings, app-language settings, runtime working-state snapshots, the runtime notification settings snapshot, and the cross-process app-visibility heartbeat now update under that same file lock. Shared JSON read-modify-write helpers now treat malformed current records as missing so the next locked update can recover a valid snapshot without store-local corrupt-read code. Scheduled run-record deletion/pruning now uses that single-file update path too, preventing stale schedule-delete snapshots from overwriting newer run records written by foreground, service, or repair flows. Runtime notification-delivery dedupe writes are covered by the same update primitive, preventing a stale foreground/service/repair snapshot from dropping another run's newer terminal-notification fingerprint. Runtime working state now also resolves from a per-session runtime-root durable snapshot in production owner assembly instead of the runtime factory's in-memory default, so host/service rebuild paths can restore prompt working state without depending on process-local caches. Chat workspace todo replacement now also reloads the current workspace record through the file-backed store update path before writing todo extensions, preventing service/runtime `TodoWrite` from re-saving an older chat workspace snapshot over newer foreground or repair-owned extensions. Runtime notification settings now also resolve from one runtime-root durable snapshot instead of multi-key `SharedPreferences`, so the main process settings UI and the `:runtime` notification coordinator share one process-safe detached-runtime source of truth. Sandbox execution settings now follow the same runtime-root durable snapshot path, with a legacy SharedPreferences migration only when the durable snapshot is empty, so runtime execution routing and service-owned settings reads do not diverge across Android processes. E2B sandbox-session resume state now follows that runtime-root durable snapshot path too, so app-process preview/session controls and runtime-process sandbox reconnect/resume do not diverge on the active session. Safety policy settings and live-context mode now follow that runtime-root durable snapshot path too, with the same legacy migration rule, so app-process settings writes and runtime-process policy/live-context reads do not diverge on automation mode, external access, tool overrides, subagent context policy, or context-mode selection. Network-search settings now follow the same runtime-root durable snapshot path, so app-process provider/API-key edits and runtime-process search-tool routing do not diverge on the enabled slots payload. MCP master settings now follow that runtime-root durable snapshot path too, so app-process settings writes and runtime-process MCP tool-surface reads do not diverge on the global enable switch. LLM config settings now follow that runtime-root durable snapshot path as well, so app-process provider/model edits, saved custom providers, and runtime-process model routing/capability-cache reads do not diverge on the active model config. Media/speech settings now follow that runtime-root durable snapshot path as well, so app-process media/STT provider edits and runtime-process media tool routing do not diverge on the active provider payload. App-language settings now follow that runtime-root durable snapshot path as well, so service-owned settings writes, projection fallback, and runtime-localized labels do not diverge on the active language. Skill install manifest refresh/check/install/remove now also merges under the locked single-file update primitive, preventing stale service-owned skills snapshots from dropping another concurrent skill entry. Mid-loop supplement append/consume is covered as well, preventing service-side consumption from re-saving an older snapshot over a supplement injected by another owner path. Transcript fallback append/replace/repair now uses the same primitive, preventing stale host rebuild or projection-fallback snapshots from dropping newer transcript events while journal-backed replay stays preferred. Projection snapshots now also save through the locked update path and preserve newer owner-lease plus interrupted-run repair collateral from the current target file, preventing stale projection flushes from erasing the lease conflict or reconnect repair evidence used by binder-unavailable diagnostics. Direct-file run journal append/list/clear now uses a session-level sidecar OS lock, disk-derived sequence allocation under that lock, and temp-file atomic moves, so journal events are no longer a known cross-owner overwrite risk.
The durable text store now also tolerates concurrent first-use directory creation: if `mkdirs()` loses a race but the directory exists afterward, the write proceeds instead of failing the runtime-process persistence path.
Same-thread nested access through another durable-storage instance now reuses the already-held lock scope instead of reacquiring Android's non-reentrant sidecar OS lock, preventing runtime projection callbacks from crashing with `Resource deadlock would occur`.
...
Agent registry create/update/select/archive now also uses the locked durable update path, preventing foreground agent management and service/runtime agent-scope reads from losing active-agent or descriptor changes through stale registry writes.
Per-agent config snapshots now also use shared durable text storage with locked update save/clear under each agent private directory, preventing service/runtime agent-scope reads from racing against direct truncate writes of `agent-config.json`.
Workspace and agent-private `SOUL.md` profile save/clear now also use shared durable text storage, and profile saves merge current durable frontmatter inside the locked update callback, preventing foreground personalization updates and service/runtime agent-scope reads from racing against direct truncate writes of soul profile documents.
Workspace voice metadata cache writes now also merge through the locked durable update path and cache reads refresh from the durable snapshot, preventing foreground and service/runtime attachment metadata backfill from dropping another owner's waveform, duration, or transcript cache entry through stale in-memory state.
Settings image asset index writes now also merge through the locked durable update path and reads refresh from the durable snapshot, preventing foreground imports and service/projection image-reference reads from dropping another owner's newly imported image asset through stale in-memory index state.
MCP server registry add/remove/enable/disable and built-in server seeding now mutate the current registry through a store-level atomic update path, preventing foreground settings and runtime/service MCP refreshes from saving stale registry instances over another owner's newer server or trust-state entry.
MCP registry direct `save`/`clear` entrypoints now also use the locked durable text update primitive, preventing fallback callers from bypassing the process-safe registry mutation path with direct `writeText`/`delete`.
Session compaction state now also exposes an atomic store update path, and runtime compaction append uses it instead of saving a caller-held pre-append snapshot, preventing concurrent foreground/service/runtime compaction work from dropping another owner's durable summary entry. Malformed compaction snapshots now load as empty state and are rewritten by the next locked update, so host rebuild or runtime-process recreate does not fail before compaction can continue.
Current chat workspace extension update: extension writes beyond todo replacement now also use the durable current-record update path for working state, pending user input enqueue/consume/clear, native web search approval, and session-scoped-state flags, preventing service/runtime chat extension writes from saving an older workspace extension map over newer foreground or repair-owned extensions.

Current chat workspace transcript update: direct message append, submitted-turn creation, and voice attachment metadata merge now also use the durable current-record update path and preserve monotonic workspace/session timestamps, preventing service/runtime chat writes from saving an older workspace snapshot over newer foreground or repair-owned extensions while adding transcript messages or backfilling media metadata.

Current chat workspace mutation update: all `ChatSessionLocalStore` workspace mutations now use the durable current-record update path instead of direct caller-held snapshot saves, covering session create/select/copy/delete, branch, recall, message insert/replace/prune/delete, transcript append, pending-input, todo, working-state, web-search approval, session-scoped-state, and voice-metadata writes. This removes the remaining known stale-snapshot overwrite path for service-owned chat/session mutations while execution is still runtime-process-bound.
...
Current managed-process repair update: scheduler repair preflight and runtime-service bootstrap/resume scans now read the file-backed process registry in passive `projection_only` mode, classify retry-scheduled or retryable connecting managed processes as `MANAGED_PROCESS_RECONNECT` evidence, and also recognize queue-stored `resume_reconnect_process` recovery metadata when only the recovery-aware queue snapshot survives. Queue-backed preflight evidence now uses the recovery-aware queue projection when run-record and journal stores are available, so stale non-terminal queue rows with durable terminal results are filtered before they can wake repair as interrupted work. The repair path now carries process id and `repairAfterEpochMs` from that queue evidence into typed reconnect repair/projection, lets same-run/task queue, run-record, or journal-tail evidence inherit the reconnect deadline, and enqueues a replacement delayed repair for future `retryAfterEpochMs` values instead of relying only on the hourly periodic pass. Runtime-service bootstrap/resume scans still project future reconnect evidence for diagnostics, but they do not resume durable-only sessions until the retry deadline is due, and their projected result now exposes `nextRepairAfterEpochMs` directly while re-registering the delayed managed-process reconnect repair from that deadline. WorkManager preflight now uses the same evidence-derived delayed repair reason, so generic future interrupted-run retry evidence is not mislabeled as managed-process reconnect.
Once that reconnect deadline is due, a retained session no longer stops at `session.resume()`: it refreshes the recovery projection against the active managed-process registry, treats the existing `reconnect_hold` as a re-evaluable interrupted state, and queues a matching checkpoint-backed `ProcessRead`/`ProcessWait` continuation when reconnect has become stable. The queue task records the new `resume_from_checkpoint` action and clears stale reconnect-backoff fields; unsafe or checkpoint-less holds remain suspended.
Malformed managed-process registry snapshots now load as empty records, so projection-only repair scans, runtime-service bootstrap, and later managed-process mutations can continue through the process-safe registry path and recover the file on the next locked save instead of failing before reconnect evidence can be considered.
...
- live managed-process restores now also fall back to that same queued checkpoint-resume path when a safe `general_resume` checkpoint exists, instead of staying in a reconnect state that cannot yet continue end-to-end; that safe checkpoint path now also carries the managed-process restore scope and restore decision into task metadata, lifecycle diagnostics, and the durable recovery marker
...
- service-backed chat/skills/settings gateways now keep fallback strictly projection-only for reads; binder-unavailable writes no longer drop back to the UI-side facade, but background-thread writes can use the target-scoped service-owned loopback command transport before surfacing a binder-unavailable failure
...
- scheduled-dispatch notifications now also have service-backed retry/manual-run, disable, and snooze actions for skipped-busy and dispatch-failed outcomes; those actions are parsed by the runtime-service wake-command pipeline, start the detached target in the foreground, and either enqueue a new manual scheduled trigger through the normal scheduled-task dispatcher, disable the durable schedule with trigger re-sync, or persist a short schedule deferral while preserving the original trigger. Accepted-run Cancel still reuses the chat-write interrupt wake, but that parser now reserves terminal stale-notification dismissal metadata for Retry only. The trigger re-sync transaction itself is now guarded by a runtime-root cross-process lock around schedule-spec readback, prior synced-id readback, cancel/sync fanout, and synced-id persistence, so app bootstrap, repair, and detached notification actions do not race each other into stale registered-trigger state.
...
- interactive and detached service ownership now live in separate target processes, and Binder v2 can carry typed writes across that boundary; projection, loopback, and wake remain explicit compatibility/failure fallbacks, while API 35 verifies real remote Binder, independent PIDs, and detached process-death recovery

Not yet implemented:

- repeat the passing API 35 remote Binder and detached-process kill/recreate harness on a physical device, then repeat the covered E2B managed-process reconnect/repair chain against a live provider endpoint
- restore the unrelated stale UI resource-id AndroidTest baseline in the UI migration track so the complete legacy instrumentation suite builds again
...
- a main app-process or host recreation does not necessarily recreate the run owner anymore, because production ownership now lives in the target runtime process
...
- controller-level managed-process reconnect is implemented for reconnectable backends, including the runtime routed-factory path when the default delegate supports reconnect; detached ownership runs in `:runtime_controller`, versioned read/write controller IPC is implemented, device-level v1 projection-only negotiation/write fallback is covered, and API 35 now passes detached process kill/recreate, one safe checkpoint-backed active-task continuation, one no-checkpoint explicit-interruption/no-replay path, and E2B provider-native reattach through a process-external Connect endpoint, while credentialed cloud-provider and physical-device verification remain open
...
- runtime-service owner-lease denial does not execute work in a losing shell; attach denial and start-command lease loss both enqueue a delayed `owner_lease_expired` repair at the held lease expiry so a later repair wake can retry without handing ownership to the loser
...
- target-scoped runtime-owner lease heartbeat now records held/released owner evidence under the runtime storage root, projects the latest lease into shell/chat diagnostics, records the latest rejected acquire attempt on the held lease, prevents a different owner from replacing a still-held unexpired lease, prevents non-owner coordinators from overwriting the active owner's target projection, mirrors ordinary disposal into projection as a released lease plus destroyed keepalive state only after the durable release still belongs to the same owner, gates shell attach, service start-command handling, sticky restart decisions, plus wake/binder writes on the held target lease, schedules owner-lease-expiry repair when service attach is denied by a still-held lease, has periodic repair preflight rebuild the earliest future owner-lease-expiry delayed repair from durable lease evidence, and wakes an already-expired still-held target through the interrupted-run rescan path instead of completing the repair worker without a target wake
...
- service-owned loopback POST routes now also check the target owner lease before dispatching runtime-owned writes, so the binder fallback HTTP path has the same non-owner mutation boundary as wake commands and binder chat/skills/settings writes
- execution is still ultimately backed by runtime-process singletons and executors
- `AlarmManager` plus `WorkManager` trigger bridges now exist for scheduled wake-up and repair, schedule trigger sync now registers the next enabled wake through both bridges, Alarm wake delivery now requeues into the WorkManager wake fallback when the foreground runtime-service start request returns `false`, WorkManager scheduled wake workers also retry rejected starts, and schedule notifications can retry/manual-run, snooze, or disable failed/skipped schedules through the runtime-service wake path, but interactive active runs still execute under that runtime-process owner
- interrupted-run repair scans now preserve typed evidence by session across WorkManager preflight and runtime-service bootstrap/resume repair results, queue-backed repair evidence is classified through the recovery-aware queue projection when possible and carries the run ownership tier with a `runtime_process` default for older metadata plus durable runtime-controller id when task metadata has one, target-scoped runtime-service projection active/pending/live session ids can provide `runtime_projection_work` fallback evidence only when no stronger durable evidence already exists for that session, and the service-side bootstrap/resume result is now saved into durable projection diagnostics with `nextRepairAfterEpochMs` plus `nextRepairReason`, used to re-register the next delayed repair under the matching WorkManager reason even when the typed projection evidence list is absent, and treated as a target wake trigger once that projection-level deadline is already due, so host rebuild and binder-unavailable fallback can tell whether a repair wake came from queue state, a prompt checkpoint, a detached subagent handle, a durable run record, the latest non-terminal journal tail, projection work-summary fallback, or managed-process reconnect backoff and which runtime ownership anchor produced the queue/projection evidence; finalization checkpoints are treated as terminal evidence and do not independently trigger repair wake
- resume-interrupted-run command envelopes now retain the normalized wake source through the service dispatcher, repair result, process-safe target projection store, and diagnostics as `requestedRepairReason`; this lets host-rebuild reports distinguish owner-lease expiry, managed-process reconnect, periodic, app-start, and generic retry triggers without overloading the evidence-derived `nextRepairReason`
- managed-process recovery rewrites now also stamp `_host.managedProcessContinuationBasis`, `_host.managedProcessRestoreScope`, and `_host.managedProcessRestoreDecision` into task metadata, lifecycle diagnostics, and durable recovery markers, so host-rebuild reports can distinguish checkpoint-backed observation continuation (`checkpoint_resume`), stable live reconnect evidence (`live_reattach`), suspended reconnect/backoff holds (`reconnect_hold`), runtime-registry restore scope (`same_controller`, `same_process_new_controller`, or `cross_process`), and registry restore decisions (`live_controller_reattached`, `reconnect_attempted`, `reconnect_deferred`, or `interrupted_no_controller`) without implying a separate controller process exists yet; interrupted-run repair evidence and the runtime-service projection store now preserve the same basis/scope/decision plus reconnect status, reconnect recovery state, and attempt count for queue-stored or registry-discovered reconnect holds, while registry-discovered retry/backoff reconnect evidence still defaults its continuation basis to `reconnect_hold`
...
- runtime notification settings save/clear now use that same process-safe durable update primitive, so foreground settings and detached notification-policy paths do not persist quiet-hours or channel-toggle snapshots through direct file writes
...
- production runtime working state now uses a per-session runtime-root file-backed store instead of the runtime factory's in-memory default, so owner replacement and runtime-process recreate can restore prompt working state through the same durable storage family as queue/checkpoint/journal evidence
- sandbox execution settings now default to the runtime-root durable update path instead of defaulting to process-local `SharedPreferences`, so settings UI saves and runtime-process sandbox routing read the same process-safe backend/session/timeout state after legacy migration
- E2B sandbox-session resume state now defaults to the runtime-root durable update path instead of defaulting to process-local `SharedPreferences`; preview/session-info lifecycle writes and runtime-process sticky reconnect now merge against the latest snapshot under the same sidecar file lock, while stale-owner cleanup is conditional on the current sandbox id, so overlapping app/runtime owners preserve newer preview, remote-workspace, and replacement-session evidence after legacy migration
- E2B sticky workspace sync state now stores the reusable local file manifest through the shared durable text storage under the workspace `.opencray/sandbox-sync` directory, so sticky sandbox reconnect and host rebuild paths no longer read/write that hint through a separate direct-file persistence path, and save/clear writes use the locked update primitive
- E2B native sandbox busy/reclaim scans now read each session's managed-process registry through the projection-only file-backed registry path, so durable sandbox liveness checks share registry locking and normalization instead of parsing a potentially stale duplicate snapshot list directly
- safety policy settings and live-context mode now default to runtime-root durable update paths instead of defaulting to process-local `SharedPreferences`, so settings UI saves and runtime-process tool-policy/live-context reads use the same process-safe automation/external-access/tool-override/subagent-context/context-mode state after legacy migration
- network-search settings now default to the runtime-root durable update path instead of defaulting to process-local `SharedPreferences`, so settings UI saves and runtime-process search-tool routing use the same process-safe provider/API-key slots state after legacy migration
- MCP master settings now default to the runtime-root durable update path instead of defaulting to process-local `SharedPreferences`, so settings UI saves and runtime-process MCP tool-surface reads use the same process-safe global enable state after legacy migration
- LLM config settings now default to the runtime-root durable update path instead of defaulting to process-local `SharedPreferences`, so settings UI saves and runtime-process model routing/capability-cache reads use the same process-safe provider/model/custom-provider state after legacy migration
...
- those runtime-root settings migrations now only suppress legacy migration when the durable snapshot decodes successfully; a malformed durable JSON file is treated like a missing durable record and can be overwritten through the locked update path, preserving host-rebuild recovery for settings that still have legacy `SharedPreferences` values
...
- checkpoint-resume loop control now increments `_host.checkpointResumeAttemptCount` only when the core queue transitions into an actual `checkpoint_resume` execution, projects it through run diagnostics and terminal result metadata, records it in recovery journal evidence, and converts the fourth would-be automatic continuation into explicit interruption; a user Retry clears the counter before the next execution
...
- per-run ownership-tier metadata versus the shell/projection ownership head
...
- same-process live-controller adoption after runtime-controller recreate versus provider reconnect or interrupted fallback, including `managedProcessRestoreDecision=live_controller_reattached`
...
- per-session interrupted-run repair evidence kind for wake/rescan decisions, including queue-task, prompt-checkpoint, detached-subagent, run-record, journal-tail-only, runtime-projection work-summary, and managed-process reconnect recovery candidates, plus the latest persisted service-side repair projection when binder fallback reads only the target-scoped projection snapshot
...
- reconnectable managed-process registry restore now stamps reconnect-attempted restore metadata after backend controller creation, so cross-process/current-controller diagnostics survive even when a provider returns a fresh live snapshot without copying reconnect input metadata
...
- same-process runtime-controller recreate can now adopt a still-live managed-process controller from a process-start-scoped registry index when no provider reconnect factory is available, so local controller churn no longer forces a durable running snapshot into interrupted repair just because the strict controller-instance scope changed
- interrupted terminal notifications can now wake the resolved runtime target through the existing `RetryChatRun` command, refresh projection state after the dispatch attempt, and clear the stale interrupted notification only after dispatch succeeds, so explicit user retry after a host rebuild or service recreate no longer depends on first reopening the chat UI while failed dispatch stays visible
- generated media artifact registration and sweep now merge under the shared durable text storage OS lock, so runtime generation and app-process media GC/resolution no longer race through a direct overwrite guarded only inside one JVM
