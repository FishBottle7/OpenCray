...
- E2B sandbox-session resume state now defaults to the runtime-root durable update path instead of defaulting to process-local `SharedPreferences`, so app-process preview/session controls and runtime-process sandbox reconnect/resume use the same process-safe active-session state after legacy migration
