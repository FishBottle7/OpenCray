...
- the binder-backed service chat gateway now also serves runtime snapshot plus memory/soul debug reads, search/slice helpers, and memory-debug actions through projection-backed local stores and service-owned observer fanout rather than delegating those surfaces through `OpenCrayHostRuntime`; the primary chat snapshot and per-run snapshot/wait surfaces remain host-backed for now because the projection surface still lacks full message/rendered-event and run-inspector metadata parity
...
- personalization load/save/reset now also terminate inside the service-owned settings gateway through `LocalPersonalizationFacade`, and app-language persistence now also flows through a service-owned app-language access seam; the remaining host coupling on that path is narrowed to localized runtime refresh plus chat/skills/settings snapshot fanout, so binder-connected reads do not keep stale language state and no longer route the language write itself back through `OpenCrayHostRuntime`
