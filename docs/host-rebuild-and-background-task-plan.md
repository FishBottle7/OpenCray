...
- interrupted-run repair scans now preserve typed evidence by session across WorkManager preflight and runtime-service bootstrap/resume repair results, queue-backed repair evidence is classified through the recovery-aware queue projection when possible and carries the run ownership tier with a `runtime_process` default for older metadata plus durable runtime-controller id when task metadata has one, and the service-side bootstrap/resume result is now saved into durable projection diagnostics with `nextRepairAfterEpochMs` plus `nextRepairReason` and used to re-register the next delayed repair under the matching WorkManager reason, so host rebuild and binder-unavailable fallback can tell whether a repair wake came from queue state, a prompt checkpoint, a detached subagent handle, a durable run record, the latest non-terminal journal tail, or managed-process reconnect backoff and which runtime ownership anchor produced the queue evidence; finalization checkpoints are treated as terminal evidence and do not independently trigger repair wake
