...
Current ownership-evidence update: each target now has a process-safe runtime-owner lease heartbeat that records held/released owner evidence with process, controller, owner, service, heartbeat, and expiry fields. Different owners can no longer overwrite a still-held unexpired lease, retained runtime-owner replacement releases the previous owner lease before saving the new owner heartbeat, a coordinator that fails to acquire the target lease skips projection writes instead of replacing the active owner's projection, and non-owner service shells skip start-command handling, wake-command dispatch, binder chat/skills/settings writes, and sticky restart requests. This narrows host-rebuild diagnostics and arbitration for retained-owner continuity versus controller teardown, but it is still for the existing `:runtime` process owner rather than a separate detached controller process.
...
- target-scoped runtime-owner lease heartbeat now writes held/released owner evidence through a process-safe durable store and mirrors the latest lease into runtime-service projection diagnostics, so host rebuild reports can identify which runtime owner/controller last claimed a target without peeking a live process-local singleton
...
- target-scoped runtime-owner lease heartbeat now records held/released owner evidence under the runtime storage root, projects the latest lease into shell/chat diagnostics, prevents a different owner from replacing a still-held unexpired lease, prevents non-owner coordinators from overwriting the active owner's target projection, and gates service start-command handling, sticky restart decisions, plus wake/binder writes on the held target lease
