...
Current controller-IPC update: the stable service binder now implements read-only `IRuntimeServiceController` v1. Remote clients negotiate protocol version and target identity, decode a bounded payload through the existing projection persistence schema, and become binder-connected instead of treating `BinderProxy` as an invalid process-local object. The remote adapter still refuses to expose shell/chat/skills/settings gateway objects, so mutating calls retain the process-neutral loopback or wake fallback until versioned command envelopes are added.
