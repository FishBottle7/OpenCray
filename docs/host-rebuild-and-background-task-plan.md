...
- managed-process recovery rewrites now also stamp `_host.managedProcessContinuationBasis` into task metadata, lifecycle diagnostics, and durable recovery markers, so host-rebuild reports can distinguish checkpoint-backed observation continuation (`checkpoint_resume`), stable live reconnect evidence (`live_reattach`), and suspended reconnect/backoff holds (`reconnect_hold`) without implying a separate controller process exists yet; interrupted-run repair evidence and the runtime-service projection store now preserve the same basis for queue-stored reconnect holds and default registry-discovered retry/backoff reconnect evidence to `reconnect_hold`
