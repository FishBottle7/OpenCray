...
Current chat workspace transcript update: direct message append and submitted-turn creation now also use the durable current-record update path and preserve monotonic workspace/session timestamps, preventing service/runtime chat appends from saving an older workspace snapshot over newer foreground or repair-owned extensions while adding transcript messages.
