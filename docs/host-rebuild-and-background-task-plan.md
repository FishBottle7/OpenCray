...
- run lifecycle metadata now also stamps the same ownership tier through `_host.runtimeExecutionOwnershipTier` plus `_host.runtimeControllerProcessSeparate`, and `RunLifecycleDiagnostics` projects those values into run snapshots
