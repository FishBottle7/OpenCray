...
- target-scoped runtime-owner lease heartbeat now records held/released owner evidence under the runtime storage root, projects the latest lease into shell/chat diagnostics, records the latest rejected acquire attempt on the held lease, prevents a different owner from replacing a still-held unexpired lease, prevents non-owner coordinators from overwriting the active owner's target projection, and gates shell attach, service start-command handling, sticky restart decisions, plus wake/binder writes on the held target lease
