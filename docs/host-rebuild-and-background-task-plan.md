...
Current process-recreate verification update: `RuntimeServiceProcessIsolationTest` now drives both real service components through remote AIDL, records their independent process identities, kills the detached process, waits out its persisted owner lease, wakes the target through the production owner-lease repair path, and compares rebuilt process/durable-controller identity. The test still requires a connected Android device or emulator before its process-death assertions can be treated as verified; the AndroidTest source set also remains blocked by pre-existing UI tests that reference removed resource ids, with no compile error currently attributed to this runtime test.
...
- restore the unrelated stale UI resource-id AndroidTest baseline, execute the checked-in remote Binder and detached-process kill/recreate instrumentation harness, then repeat the covered E2B managed-process reconnect/repair chain on Android against a live provider endpoint
