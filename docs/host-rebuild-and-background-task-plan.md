...
- managed-process recovery rewrites now also stamp `_host.managedProcessContinuationBasis` and `_host.managedProcessRestoreScope` into task metadata, lifecycle diagnostics, and durable recovery markers, so host-rebuild reports can distinguish checkpoint-backed observation continuation (`checkpoint_resume`), stable live reconnect evidence (`live_reattach`), suspended reconnect/backoff holds (`reconnect_hold`), and runtime-registry restore scope (`same_controller`, `same_process_new_controller`, or `cross_process`) without implying a separate controller process exists yet; interrupted-run repair evidence and the runtime-service projection store now preserve the same basis/scope plus reconnect status, reconnect recovery state, and attempt count for queue-stored or registry-discovered reconnect holds, while registry-discovered retry/backoff reconnect evidence still defaults its continuation basis to `reconnect_hold`
