...
Current ownership-evidence update: each target now has a process-safe runtime-owner lease heartbeat that records held/released owner evidence with process, controller, owner, service, heartbeat, and expiry fields. Different owners can no longer overwrite a still-held unexpired lease, and retained runtime-owner replacement releases the previous owner lease before saving the new owner heartbeat. This narrows host-rebuild diagnostics and arbitration for retained-owner continuity versus controller teardown, but it is still for the existing `:runtime` process owner rather than a separate detached controller process.
...
- target-scoped runtime-owner lease heartbeat now records held/released owner evidence under the runtime storage root, projects the latest lease into shell/chat diagnostics, and prevents a different owner from replacing a still-held unexpired lease
