...
- safety policy settings now default to the runtime-root durable update path instead of defaulting to process-local `SharedPreferences`, so settings UI saves and runtime-process tool-policy reads use the same process-safe automation/external-access/tool-override/subagent-context state after legacy migration
