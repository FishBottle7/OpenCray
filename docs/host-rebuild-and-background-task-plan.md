...
Session compaction state now also exposes an atomic store update path, and runtime compaction append uses it instead of saving a caller-held pre-append snapshot, preventing concurrent foreground/service/runtime compaction work from dropping another owner's durable summary entry.
