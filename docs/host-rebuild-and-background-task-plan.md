...
- interactive and detached service ownership now live in separate target processes, and Binder v2 can carry typed writes across that boundary; projection, loopback, and wake remain explicit compatibility/failure fallbacks while device-level remote Binder and process-death behavior are still being verified

Not yet implemented:

- execute the independently compiled remote Binder and detached-process kill/recreate harness on an emulator and physical device, then repeat the covered E2B managed-process reconnect/repair chain against a live provider endpoint
