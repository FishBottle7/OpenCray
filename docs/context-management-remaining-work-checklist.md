...
1. Structured soul profile
   ...
   - Current status: typed interaction-preference planning is now structurally thinner than before. During post-turn ingestion, `InteractionPreferenceState` snapshots are planned directly from the turn's extracted preference candidates, with compatibility records retained for inspection and gradual phase-down, so typed state generation no longer depends on re-reading freshly written legacy preference records as an intermediate source of truth.
   - Current status: durable `relationship_style_profile` compatibility records now also carry internal typed interaction-preference signal extensions, and `InteractionPreferenceMemoryWritePlanner` prefers those typed signal fields over the legacy `warm` / `serious` value mapping when projecting the next snapshot. That means the compatibility key can keep existing write contracts while no longer acting as the sole semantic carrier for preference drift.
   - Current status: the LiteLLM user/soul intent interpreters and runtime intent models now also expose a dedicated `preference_extensions` channel for compatibility-style relationship writes, so upstream extraction can emit typed adaptive-direction hints explicitly instead of relying only on `relationship_style_profile=warm|serious`. The old compatibility key remains in place, but the typed direction fields are now part of the intended write contract.
   - Current status: write-side extraction now also supports an explicit `interaction_preference_signal` preference key as the preferred typed carrier for adaptive warmth/formality/initiative drift. Runtime canonicalizes its durable value from `preference_extensions`, projects it into `InteractionPreferenceState`, and keeps `relationship_style_profile` only as a legacy compatibility path for coarse old-style labels.
