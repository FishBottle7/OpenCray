...
- E2B sticky workspace sync state now also resolves through the shared durable text storage under each workspace's `.opencray/sandbox-sync` directory, so reusable sync manifests survive runtime-process churn with the same sidecar file-lock plus atomic-replace behavior as the other strong-background stores
...
- E2B sticky workspace sync state now also uses that shared durable text storage under each workspace's `.opencray/sandbox-sync` directory, so reusable local file manifests for sticky sandbox reconnect are no longer persisted by a separate direct-file writer
