...
Current process-death verification status: an Android instrumentation harness now covers simultaneous `:runtime`/`:runtime_controller` remote binding, v2 write transport, detached-process kill, stale owner-lease expiry, production `owner_lease_expired` repair wake, and rebuilt-owner identity checks. The harness now lives in an independently selectable AndroidTest source directory, and `:app:compileDebugAndroidTestKotlin -PruntimeIsolationAndroidTestOnly=true` passes without compiling unrelated stale UI tests. No emulator or device is currently attached, so this remains compiled-but-not-executed evidence rather than completed device validation; the full legacy AndroidTest source set still needs its separate UI resource-id migration.
...
- run `./gradlew :app:connectedDebugAndroidTest -PruntimeIsolationAndroidTestOnly=true -Pandroid.testInstrumentationRunnerArguments.class=com.opencray.app.RuntimeServiceProcessIsolationTest` on an emulator and at least one physical Android device, then retain the observed process/lease/reconnect diagnostics with the release evidence
