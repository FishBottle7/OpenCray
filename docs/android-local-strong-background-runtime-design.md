...
Current process-death verification status: the independently selectable Android instrumentation harness covers simultaneous `:runtime`/`:runtime_controller` remote binding, v2 write transport, detached-process kill, stale owner-lease expiry, production `owner_lease_expired` repair wake, rebuilt-owner identity checks, detached-to-main scheduled-work delivery, and one safe checkpoint-backed active-task continuation. All four cases pass on the API 35 `OpenCrayRuntimeIsolationApi35` x86_64 emulator (`4` tests, `0` failures, `0` skipped), proving real remote `BinderProxy`, independent PIDs, rebuilt process/owner identity, durable controller identity continuity, a stable main-process WorkManager wake produced by a `:runtime_controller` schedule mutation, and same-task/run `checkpoint_resume` from a persisted parsed-action boundary after the detached PID is killed. Physical-device repetition remains release evidence; the unrelated legacy AndroidTest UI resource-id migration remains a separate baseline task.
