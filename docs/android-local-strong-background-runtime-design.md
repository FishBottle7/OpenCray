...
- recovery diagnostics now stamp run attempts and checkpoint ids on recovery-aware queue rewrites, so strong-background reports can distinguish an initial submission from a checkpoint-backed restore without relying only on queue state
...
- recovery-aware queue restore now also exposes `runAttempt` and checkpoint-backed `recoveredFromCheckpointId` through run lifecycle diagnostics, improving detached/background debugging without changing the remaining runtime-process ownership boundary
