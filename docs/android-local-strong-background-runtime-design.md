...
Current process-death verification status: the independently selectable Android instrumentation harness covers simultaneous `:runtime`/`:runtime_controller` remote binding, v2 write transport, detached-process kill, stale owner-lease expiry, production `owner_lease_expired` repair wake, rebuilt-owner identity checks, and detached-to-main scheduled-work delivery. All three cases pass on the API 35 `OpenCrayRuntimeIsolationApi35` x86_64 emulator (`3` tests, `0` failures, `0` skipped), proving real remote `BinderProxy`, independent PIDs, rebuilt process/owner identity, durable controller identity continuity, and a stable main-process WorkManager wake produced by a `:runtime_controller` schedule mutation. Physical-device repetition remains release evidence; the unrelated legacy AndroidTest UI resource-id migration remains a separate baseline task.
