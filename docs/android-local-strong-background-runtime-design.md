...
- active-runtime, approval-needed, completion/interruption, and scheduled-dispatch notifications now exist, including notification-side approve/reject entry for approval waits and target-scoped Retry for interrupted terminal runs
...
- short reason
- `Review`
- `Retry`, routed as a target-scoped `RetryChatRun` runtime-service wake command
...
- interrupted terminal notification Retry actions that route through the same chat-write wake command model and the run's resolved runtime target instead of holding direct object references from the notification
...
- interrupted terminal notifications now expose Retry through a target-scoped service PendingIntent for the existing chat-write retry wake command, so user-driven retry does not require reopening the UI first
