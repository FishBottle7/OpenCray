...
- E2B sticky sandbox busy/reclaim checks now scan durable managed-process registries through projection-only registry reads rather than direct JSON parsing, so stale duplicate running snapshots cannot make a reclaimed sandbox look busy after a newer terminal registry entry exists
