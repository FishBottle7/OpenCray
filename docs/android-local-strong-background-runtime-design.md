...
- partially implemented; interactive ownership runs in `:runtime`, detached/scheduled ownership runs independently in `:runtime_controller`, target-aware bootstrap rejects mismatched component targets and process placement before owner creation, and versioned read/write controller IPC now spans that boundary. API 35 remote-Binder and detached process kill/recreate validation pass; remaining isolation evidence is physical-device repetition plus real-provider managed-process reconnect/repair across runtime-process death.

- retain the passing `IRuntimeServiceController` v2 remote-`BinderProxy`/independent-process evidence and add device-level v1 projection-only negotiation plus capability-fallback coverage if v1 compatibility remains a release requirement
