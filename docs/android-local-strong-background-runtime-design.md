...
- runtime notification settings now also resolve from one runtime-root durable snapshot instead of cross-process `SharedPreferences`, so the main-process notification settings surface and the detached `:runtime` notification coordinator read one process-safe source of truth for quiet hours, delivery mode, and per-event notification toggles
...
- skill install manifest refresh/check/install/remove now also merges through the durable single-file update primitive, so service-owned skills commands from overlapping app/runtime paths do not overwrite another skill's newer manifest entry with a stale snapshot
...
- managed-process reconnect recovery and repair evidence now also carries restore scope plus reconnect status details: recovery-aware queue rewrites stamp `_host.managedProcessRestoreScope`, lifecycle diagnostics and durable recovery markers project it, and runtime-service repair projection persists `managedProcessRestoreScope`, `managedProcessReconnectStatus`, `managedProcessReconnectRecoveryState`, and `managedProcessReconnectAttemptCount` for queue-stored or registry-discovered reconnect holds, so strong-background diagnostics can distinguish `same_controller`, `same_process_new_controller`, and `cross_process` evidence plus `connecting`/`retry_scheduled` backoff state while controller-process ownership remains pending
