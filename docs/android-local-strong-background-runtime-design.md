...
- active-runtime, approval-needed, completion/interruption, and scheduled-dispatch notifications now exist, including notification-side approve/reject entry for approval waits and target-scoped Retry for interrupted terminal runs with service-side stale-notification dismissal
...
- short reason
- `Review`
- `Retry`, routed as a target-scoped `RetryChatRun` runtime-service wake command that dismisses the stale interrupted notification after dispatch
...
- interrupted terminal notifications now expose Retry through a target-scoped service PendingIntent for the existing chat-write retry wake command, and the service dispatcher dismisses that stale interrupted notification after retry dispatch so user-driven retry does not require reopening the UI first
