...
- file-backed managed-process registries can now reattach live controllers across registry or host rebuild while the same app process remains alive, and that restore path is now scoped by runtime controller identity instead of only durable directory; reconnectable managed-process backends now also have a tested cross-owner restore path when a rebuilt runtime owner reopens the same durable session directory, and runtime-level managed-process routing now forwards non-Python-runtime reconnect to a reconnectable default delegate instead of masking that capability behind the Python adapter route; registry read/modify/write operations now also use a directory-scoped JVM lock plus OS file lock so concurrent detached owners or projection readers do not overwrite each other's managed-process snapshots, although the overall detached runtime is still only isolated to the dedicated `:runtime` process rather than a stronger controller tier
...
- host/runtime run snapshots and projection-only chat fallback now project the same managed-process auto-resume eligibility evidence as recovery-aware restore, so strong-background diagnostics can tell stable attached live processes apart from reconnect backoff before presenting checkpoint-resume versus interruption state
...
- interrupted-run repair preflight and runtime-service rescan now share typed `InterruptedRunRepairEvidence` by session, so strong-background diagnostics and later policy can distinguish queue-task, prompt-checkpoint, detached-subagent, run-record, and journal-tail repair reasons; the latest bootstrap/resume repair projection is persisted into the target-scoped runtime-service projection snapshot and exposed through shell/chat diagnostics when binder access is unavailable; journal-tail-only evidence can route to the detached background target when it matches a scheduled/detached queue task, and finalization checkpoints no longer wake repair by themselves
- target-scoped runtime-owner lease heartbeat now persists held/released owner evidence through the same runtime storage root and projection diagnostics, including process/controller/owner/service ids plus heartbeat/expiry timestamps; the lease store also rejects different-owner replacement while a held lease is still unexpired, retained runtime-owner replacement releases the old lease before writing the replacement heartbeat, coordinators that lose the target lease do not persist projection snapshots over the active owner, and non-owner service shells skip `onStartCommand(...)`, wake-command dispatch, and binder chat/skills/settings writes instead of mutating runtime-owned state, so strong-background reports can distinguish retained-owner continuity from controller teardown without consulting a live in-process singleton
