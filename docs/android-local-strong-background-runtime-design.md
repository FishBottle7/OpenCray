...
- managed-process reconnect recovery and repair evidence now also carries restore scope: recovery-aware queue rewrites stamp `_host.managedProcessRestoreScope`, lifecycle diagnostics and durable recovery markers project it, and runtime-service repair projection persists the same `managedProcessRestoreScope` for queue-stored or registry-discovered reconnect holds, so strong-background diagnostics can distinguish `same_controller`, `same_process_new_controller`, and `cross_process` evidence while controller-process ownership remains pending
