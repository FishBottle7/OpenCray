...
- chat workspace transcript append paths for direct message append and submitted-turn creation now also use that durable current-record update path and keep workspace/session timestamps monotonic, so detached service/runtime chat appends cannot overwrite newer foreground or repair-owned workspace extensions while adding transcript messages
