...
- safety policy settings now also resolve from a runtime-root durable snapshot instead of defaulting to cross-process `SharedPreferences`; old preference values are migrated once when the durable snapshot is empty, so app-process settings saves and `:runtime` tool-policy reads share one automation/external-access/tool-override source of truth
