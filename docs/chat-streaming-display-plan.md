...
Implemented in `feat/chat-streaming-display` worktree.

## Implementation Status

The current worktree now implements the Phase 1 design in code.

Completed:

- gateway requests can carry a live visible-text observer
- provider streaming emits throttled full-so-far assistant text snapshots
- runtime forwards live assistant drafts without writing partial text into transcript or run journal
- host runtime keeps per-session live drafts in memory only
- Flutter replaces the existing pending `Thinking` bubble in place by `pendingMessageId`
- final assistant persistence still happens exactly once at normal completion
- LLM settings now include a global `streamingEnabled` switch for chat-visible streaming
- real-device chat observation now keeps the runtime-service binder retained while chat streams are being observed, so the UI does not silently fall back to projection-only snapshots after the idle binding timeout

Current provider support:

- Anthropic and Kimi SSE: enabled
- OpenAI chat completions SSE: enabled when route metadata sets `stream=true`
- OpenAI Responses SSE: enabled when route metadata sets `stream=true`

Deliberately unchanged:

- tool execution, approval flow, transcript durability, and run journal remain non-streaming
- structured parsing still waits for the full reconstructed payload
- partial drafts are not restored after process death
- turning the switch off reverts the app to final-only assistant display without changing the non-chat surfaces

Verified:

- `.\.codex-worktrees\chat-streaming-display\gradlew.bat :app:testDebugUnitTest`
- `.\.codex-worktrees\chat-streaming-display\gradlew.bat :runtime:testDebugUnitTest`
- `.\gradlew.bat :app:testDebugUnitTest --tests com.opencray.app.OpenCrayRuntimeServiceHostTest`
- `.\build-apk.ps1 -Variant debug`

Known validation gap:

- Flutter-side tests and `dart` or `flutter` verification are still environment-constrained in the current sandbox and were not rerun in this pass
- real-device validation of streaming display and interrupt-after-streaming is still pending because no device was attached when the latest APK was rebuilt

## 2026-04-07 Device Fix Note

During real-device investigation, one additional issue was identified after the main streaming implementation had already landed.

The chat UI on device reads runtime snapshots through `ServiceBackedOpenCrayChatRuntimeGateway`.

Before this fix, `observeChat(...)` and `observeChatRuntime(...)` registered only passive connection observation. That meant the runtime-service client was free to release its binder after the idle timeout, even while the chat page was actively observing updates.

Once the binder dropped, the chat runtime gateway could fall back to the projection-only snapshot source. That fallback does not carry in-memory `liveAssistantDrafts`, so the user would stop seeing the streamed assistant text and instead wait for the final persisted answer.

The fix was narrow:

- keep chat observation on the retained connection observation path
- add regression tests that verify both chat snapshot observation and chat-runtime observation re-check the gateway after retained observation begins

This change is intentionally limited to the chat observation path. It does not change the non-chat surfaces, tool execution model, or final persistence rules.
...
The points below record the baseline repository findings that motivated the design.

They describe the pre-change state that this worktree set out to change. For the implemented state, use the `Implementation Status` section above as the source of truth.
